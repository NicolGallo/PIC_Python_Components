...
class MqttClientConnector(IPubSubClient):
	...
	def __init__(self, clientID: str = None):
		...
		self.defaultCleanSession = self.config.getBoolean(ConfigConst.MQTT_GATEWAY_SERVICE,
														  ConfigConst.DEFAULT_CLEAN_SESSION_KEY,
	...
	def connectClient(self) -> bool:
		if not self.mqttClient:
			#TODO Comprobar como hacer para que el callback API version 1 no este deprecado.

			self.mqttClient = mqttClient.Client(client_id=self.clientID, clean_session=self.defaultCleanSession)
	...
	def onMessage(self, client, userdata, msg:MQTTMessage):
		...
		#To test pub/sub using a JSON payload, is necessary to decode the byte array with an appropriate scheme like utf-8
		if payload:
